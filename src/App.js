...
// Demo fallback helpers (used when using demo admin token)
...
const DEMO_SAMPLE_BOOKS = [
  {
    ...
    author: 'Demo Author',
    price: 9.99,
    description: 'A sample demo book for local testing.',
    cover_image_url: 'https://via.placeholder.com/300x400?text=Demo+Book',
    stock_quantity: 10,
    category: 'Fiction'
  }
];
...
const loadDemoBooks = () => {
  try {
    return JSON.parse(localStorage.getItem('demoBooks') || 'null') || [];
  } catch (e) {
    return [];
  }
};

const saveDemoBooks = (books) => {
  localStorage.setItem('demoBooks', JSON.stringify(books));
};

const ensureDemoData = () => {
  const existing = loadDemoBooks();
  if (!existing || existing.length === 0) {
    saveDemoBooks(DEMO_SAMPLE_BOOKS.slice());
  }
};
        ...
        <div className="flex items-center space-x-6">
          {!user && showAuthLinks && (
            <>
              ...
              <Button variant="ghost" onClick={() => navigate('/auth')}>Publish</Button>
...
const HomePage = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-rose-50">
      ...
      <section className="py-16 px-6">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-4xl font-bold text-center text-slate-800 mb-12">Messages from Our Leadership</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <Card>
              <CardHeader>
                <CardTitle>Message from our CEO</CardTitle>
                <CardDescription>Leadership note</CardDescription>
              </CardHeader>
              <CardContent>
                <p className="text-muted-foreground leading-relaxed">
                  Welcome to our bookstore. Our mission is to connect readers with stories that inspire, educate, and
                  entertain. We are committed to offering a diverse selection of books and creating an excellent
                  experience for every customer. Thank you for being part of our journey.
                </p>
                <div className="mt-4 text-sm text-slate-600 font-medium">— Jane Doe, CEO</div>
              </CardContent>
            </Card>

            <Card>
              <CardHeader>
                <CardTitle>Message from our Director</CardTitle>
                <CardDescription>Director's note</CardDescription>
              </CardHeader>
              <CardContent>
                <p className="text-muted-foreground leading-relaxed">
                  Our team works tirelessly to bring you the best selection of books and a seamless shopping experience.
                  We value your feedback and strive to continuously improve our services. Happy reading!
                </p>
                <div className="mt-4 text-sm text-slate-600 font-medium">— John Smith, Director</div>
              </CardContent>
            </Card>
          </div>
        </div>
...
const AuthPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Normalize response values and set auth
      const tokenVal = response.data.access_token || response.data.token || response.data.accessToken || null;
      const respUserId = response.data.user_id || response.data.id || 'user';
      const rawIsAdmin = response.data.is_admin !== undefined ? response.data.is_admin : false;
      const respIsAdmin = rawIsAdmin === true || rawIsAdmin === 'true' || rawIsAdmin === 1 || rawIsAdmin === '1';

      // Only persist if token present
      if (!tokenVal) {
        throw new Error('No access token returned from server');
      }

      login(tokenVal, String(respUserId), respIsAdmin);
      toast({
        title: "Success!",
  ...
  const handleAdminLogin = async (e) => {
    ...
    setLoading(true);
    try {
      // Demo fallback: allow local demo admin credentials without backend
      if (formData.adminUsername === 'admin' && formData.adminPassword === 'bookstore2025') {
        const demoToken = 'demo-admin-token';
        const demoId = 'admin';
        login(demoToken, demoId, true);
        toast({ title: 'Success!', description: 'Demo admin logged in locally' });
        // Small defer to ensure AuthContext updates before route guard checks
      ...
      // Prefer returning user id from response if available, otherwise keep 'admin'
      const adminId = response.data.user_id || 'admin';
      const isAdminFlag = response.data.is_admin !== undefined ? response.data.is_admin : true;

      const tokenVal = response.data.access_token || response.data.token || response.data.accessToken || null;
      if (!tokenVal) {
        throw new Error('No access token returned from admin login');
      }

      // Ensure boolean isAdmin
      const rawIsAdmin = response.data.is_admin !== undefined ? response.data.is_admin : isAdminFlag;
      const coercedIsAdmin = rawIsAdmin === true || rawIsAdmin === 'true' || rawIsAdmin === 1 || rawIsAdmin === '1';

      login(tokenVal, String(adminId), coercedIsAdmin);
      toast({
        title: "Success!",
        description: "Admin logged in successfully",
      });

      // Defer navigation to allow AuthContext state update before route guard checks
      setTimeout(() => navigate('/admin'), 50);

      // Debug: show what was stored (helps with verifying in-browser behavior)
      setTimeout(() => {
        toast({
          title: 'Debug',
          description: `token: ${localStorage.getItem('token')}, userId: ${localStorage.getItem('userId')}, isAdmin: ${localStorage.getItem('isAdmin')}`,
        });
      }, 100);
    } catch (error) {
      toast({
  ...
  // If auth state changes to an admin user, navigate to /admin
  ...
  }, [currentUser, currentIsAdmin, navigate]);
...
const CustomerDashboard = () => {
  ...
  const fetchBooks = async () => {
    try {
      // If demo admin token is in use (local demo), load demo books from localStorage
      const token = localStorage.getItem('token');
      if (token === DEMO_ADMIN_TOKEN) {
        ensureDemoData();
        setBooks(loadDemoBooks());
        return;
      }
...
const AdminDashboard = () => {
  ...
  const handleBookSubmit = async (e) => {
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      if (token === DEMO_ADMIN_TOKEN) {
        // persist to local demo store
        const current = loadDemoBooks();
        if (editingBook) {
          const updated = current.map(b => b.id === editingBook.id ? { ...b, ...bookData } : b);
          saveDemoBooks(updated);
        } else {
          const newBook = { id: `demo-${Date.now()}`, ...bookData };
          saveDemoBooks([newBook, ...current]);
        }
      } else {
        if (editingBook) {
